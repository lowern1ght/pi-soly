...
export async function initializeMcp(
  ...
  // Surface session-recovery events via the framed notification widget.
  // Wired only when UI is available (otherwise the callback is a no-op).
  if (ctx.hasUI && ctx.ui) {
    manager.onSessionRecover = (serverName, ok, errorMsg) => {
      if (ok) {
        notifySessionRecovered(ctx.ui as ExtensionUIContext, serverName, "tools/call");
      } else {
        notifySessionRecoveryFailed(ctx.ui as ExtensionUIContext, serverName, errorMsg);
      }
    };
  }
